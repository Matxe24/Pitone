...
def main():
    if len(sys.argv) != 2:
        print("❌ Correct use: python3 pitonec.py filename.pi")
        sys.exit(1)

    file_pi = sys.argv[1]
    if not file_pi.endswith(".pi"):
        print("❌ Error: the file needs .pi extention")
        sys.exit(1)
    ...
    try:
        ...
        print(f"❌ Error: file '{file_pi}' not found.")
        sys.exit(1)
    ...
    print(f"✅ Compilated succesfully: '{nome_output}' generated.")
    avvia = input("Execute?(Y/n): ")
